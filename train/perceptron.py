...
class Perceptron:
    ...
    def train(self, sentences, save_loc=None, nr_iter=5, shuf=False):
        ...
        for iter_ in range(nr_iter):
            ...
            print("Iter {0}: {1}/{2}={3}".format(iter_, c, n,
                                                 (float(c) / n) * 100))
            self.save(save_loc)
    ...
    def save(self, loc='model/ap.model', zip=True):
        if zip == False:
            pickle.dump((self.model.weights, self.model.classes),
                        open(loc, 'wb'))
        else:
            pickle.dump((self.model.weights, self.model.classes),
                        gzip.open(loc, 'wb'))
    ...
    def load(self, loc='model/ap.model', zip=True):
        if zip == False:
            ...
            self.model.weights, self.model.classes = pickle.load(
                    gzip.open(loc,'rb'))
    ...
    def _get_features(self, i, word, context, prev, prev2):
        ...
        # It's useful to have a constant feature, which acts
        # sort of like a prior.
        add('bias')
        add('i suffix', word[-3:])
        add('i pref1', word[0])
        add('i-1 tag', prev)
